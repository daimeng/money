...
def test_parse_empty():
    res = parse_entry('', command_list=TEST_COMMANDS)
    assert res == ('Invalid command "".', None, None)


def test_parse_foo():
    res = parse_entry('foo', command_list=TEST_COMMANDS)
    assert res == (None, 'foo', {})
...
def test_parse_bar():
    res = parse_entry('bar a', command_list=TEST_COMMANDS)
    assert res == ('Usage: bar <test> <baz>', None, None)

    res = parse_entry('bar a b', command_list=TEST_COMMANDS)
